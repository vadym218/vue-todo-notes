  ...
  <div class="page">
    ...
    <div>
      <div v-if="selectedNote" class="scrollable">
        <v-text-field :rules="[value => !!value || 'don\'t leave it empty']" hint="your note name here" :value="selectedNote.name" @blur="e => updateNoteName(e.target.value)" class="note-name" />
        <div v-for="(todo, index) in selectedNote.todos" :key="index" class="todo">
          <v-checkbox :input-value="todo.done" :key="todo.done" @change="updateTodoStatus(index)" hide-details color="#888" />
